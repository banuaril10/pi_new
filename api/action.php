...
function get_data_promo_tebus($org){
	
	$curl = curl_init();

	curl_setopt_array($curl, array(
	CURLOPT_URL => "https://pi.idolmartidolaku.com/api/action.php?modul=inventory&act=sync_promo_tebus&org_id=".$org,
	CURLOPT_RETURNTRANSFER => true,
	CURLOPT_ENCODING => '',
	CURLOPT_MAXREDIRS => 10,
	CURLOPT_TIMEOUT => 0,
...
locator_name) VALUES (
		...
		foreach($j_hasil as $r) {
				...
				$upcount = $connec->query("update pos_mproduct set barcode='".$r['value']."', shortcut = '".$r['upc']."', name = '".$r['name']."' where sku='".$r['sku']."'");
